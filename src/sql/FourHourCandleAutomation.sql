-- ====================================================
-- FULLY AUTOMATED ZERO-LOSS HLCV SOLUTION FOR 4-HOUR CANDLES
-- ====================================================

-- 1. Staging Table for 4-Hour Candle Mapping
CREATE TABLE IF NOT EXISTS trade_candle_mapping_4h (
    trade_id String,
...
-- 3. Materialized View: Trade to 4-Hour Mapping
DROP VIEW IF EXISTS trade_to_mapping_4h;

CREATE MATERIALIZED VIEW IF NOT EXISTS trade_to_mapping_4h
TO trade_candle_mapping_4h
AS
SELECT
    trade_id,
    contract_address,
    toDateTime64(toStartOfInterval(trade_timestamp, INTERVAL 4 hour), 3) AS candle_timestamp
FROM trade;

-- 4. Materialized View: 4-Hour Mapping to Candles
DROP VIEW IF EXISTS mapping_to_candles_4h;

CREATE MATERIALIZED VIEW IF NOT EXISTS mapping_to_candles_4h
TO candles_4h
AS
WITH 
    trade_mappings AS (
        ...
        FROM trade_candle_mapping_4h tcm
    ),
    trade_data AS (
        SELECT
            tm.trade_id,
            ...
            addHours(tm.candle_timestamp, 4) AS end_timestamp,
            t.tokenPrice,
            t.trade_timestamp,
            t.volume
...
SELECT
    concat(ca.contract_address, ':4h:', toString(ca.timestamp)) AS candle_id,
    ca.contract_address,
    ca.timestamp,
    ca.end_timestamp,
    -- Open price logic
...
LEFT JOIN candles_4h c ON ca.contract_address = c.contract_address AND ca.timestamp = c.timestamp;

-- 5. Verification: 4-Hour Trade Audit View
CREATE OR REPLACE VIEW trade_audit_4h AS
WITH 
    trade_counts AS (
        SELECT
            contract_address,
            toDateTime64(toStartOfInterval(trade_timestamp, INTERVAL 4 hour), 3) AS timestamp,
            count() AS raw_count,
...
-- 6. Update the Data Accounting View to include 4-hour candles
...
-- OPTIMIZE TABLE trade_candle_mapping_4h FINAL;
...
-- 2. Materialized views automatically process trades into candles_4h.
-- 3. Monitor integrity with:
--    SELECT * FROM data_accounting;
--    SELECT * FROM trade_audit_4h;
