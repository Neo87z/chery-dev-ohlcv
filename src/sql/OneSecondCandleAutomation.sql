-- ====================================================
-- FULLY AUTOMATED ZERO-LOSS HLCV SOLUTION
-- ====================================================
...
-- 2. Staging Table for Trade-to-Candle Mapping
...
-- 3. Final Candles Table (1-second)
...
-- 4. Materialized View: Trade to Mapping
...
-- 5. Materialized View: Mapping to Candles
...
WITH 
    ...
    -- Aggregate OHLC data per candle
...
SELECT
    concat(ca.contract_address, ':', toString(ca.timestamp)) AS candle_id,
    ca.contract_address,
    ca.timestamp,
    ca.end_timestamp,
    -- Open price logic
...
-- 6. Verification: Data Accounting View
...
UNION ALL
SELECT
    'Trades in mapping table' AS metric,
    count() AS value
...
UNION ALL
SELECT
    'Total volume in candles' AS metric,
    sum(volume) AS value
FROM candles_1s
UNION ALL
SELECT
...
UNION ALL
SELECT
    'Trade count in candles' AS metric,
    sum(trade_count) AS value
FROM candles_1s
UNION ALL
SELECT
    'Missing trades' AS metric,
    (SELECT count() FROM trade) - (SELECT sum(trade_count) FROM candles_1s) AS value;

-- 7. Verification: Trade Audit View
CREATE OR REPLACE VIEW trade_audit AS
WITH 
    trade_counts AS (
        SELECT
            contract_address,
...
-- 2. Materialized views automatically process trades into candles_1s.
